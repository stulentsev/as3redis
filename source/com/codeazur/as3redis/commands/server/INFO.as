...
public class INFO extends SimpleCommand {
    ...
    override public function toString():String {
        ...
        if (params[ROLE]) {
            s += "\r  Role: " + params[ROLE];
        }
        if (params[REDIS_VERSION]) {
            s += "\r  RedisVersion: " + params[REDIS_VERSION];
        }
        if (params[USED_MEMORY]) {
            s += "\r  UsedMemory: " + params[USED_MEMORY];
        }
        if (params[UPTIME_IN_SECONDS]) {
            s += "\r  UptimeInSeconds: " + params[UPTIME_IN_SECONDS];
        }
        if (params[UPTIME_IN_DAYS]) {
            s += "\r  UptimeInDays: " + params[UPTIME_IN_DAYS];
        }
        if (params[CONNECTED_CLIENTS]) {
            s += "\r  ConnectedClients: " + params[CONNECTED_CLIENTS];
        }
        if (params[CONNECTED_SLAVES]) {
            s += "\r  ConnectedSlaves: " + params[CONNECTED_SLAVES];
        }
        if (params[LAST_SAVE_TIME]) {
            s += "\r  LastSaveTime: " + (new Date(parseFloat(params[LAST_SAVE_TIME]) * 1000));
        }
        if (params[CHANGES_SINCE_LAST_SAVE]) {
            s += "\r  ChangesSinceLastSave: " + params[CHANGES_SINCE_LAST_SAVE];
        }
        if (params[BGSAVE_IN_PROGRESS]) {
            s += "\r  BGSaveInProgress: " + params[BGSAVE_IN_PROGRESS];
        }
        if (params[TOTAL_CONNECTIONS_RECEIVED]) {
            s += "\r  TotalConnectionsReceived: " + params[TOTAL_CONNECTIONS_RECEIVED];
        }
        if (params[TOTAL_COMMANDS_PROCESSED]) {
            s += "\r  TotalCommandsProcessed: " + params[TOTAL_COMMANDS_PROCESSED];
        }
